...
Here, we generate many chaotic stimuli but with
different timescales

Created by Nirag Kadakia at 15:37 06-01-2022
...
for stim_scale in [1, 5, 10, 15, 20, 50, 100]:
	np.random.seed(0)
	x0 = np.random.uniform(-5, 5, 10)
	stim_Tt = np.arange(0, 100, 0.005)
	...
	for sigma in [5]:
		...
		np.save('ML_chaotic_data_sigma=%.1f_stim_scale=%d.npy' 
				% (sigma, stim_scale), data_to_save)
		data_to_save = np.vstack((Tt, res.T)).T
		np.save('ML_chaotic_true_stim_scale=%d.npy' 
				% stim_scale, data_to_save)
