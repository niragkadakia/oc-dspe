...
dynamical state and parameter estimation. This uses
tighter control on the parameters bounds.

Created by Nirag Kadakia at 08:52 09-09-2020
...
np.random.seed(seed)
P0 = [np.array([np.random.uniform(10, 30)])] + \
		 [np.array([np.random.uniform(10, 30)])] + \
		 	[np.array([np.random.uniform(0, 5)])] 
