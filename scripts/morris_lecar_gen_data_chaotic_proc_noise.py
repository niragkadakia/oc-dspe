...
This script adds process noise to the simulation,
which can affect spike timing

Created by Nirag Kadakia at 23:02 05-31-2022
...
# Standard deviation of the process noise for V and w, resp.
proc_noise_SD = 100
...
dt = 0.05
Tt = np.arange(0, 100, dt)
def stim(t):
	return np.interp(t, stim_vec_Tt, stim_vec) 

np.random.seed(0)
noise_i_V = np.random.normal(0, proc_noise_SD, len(Tt))
def I_noise(t):
	return np.interp(t, Tt, noise_i_V)
...
for sigma in [0.5, 2, 5, 10]:
	...
	np.save('ML_chaotic_data_sigma=%.1f_proc_noise=%.1f.npy' 
			% (sigma, proc_noise_SD), data_to_save)
	data_to_save = np.vstack((Tt, res.T)).T
	np.save('ML_chaotic_true_proc_noise=%.1f.npy' 
			% proc_noise_SD, data_to_save)
