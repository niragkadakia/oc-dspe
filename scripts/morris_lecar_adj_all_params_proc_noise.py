...
This script uses data that has process noise in the
observations.

Created by Nirag Kadakia at 22:37 05-31-2021
...
proc_noise_sigma = float(sys.argv[4])
...
_data = np.load('%s/meas_data/ML_%s_data_sigma=%.1f_proc_noise=%.1f.npy' %
				(data_dir, I_type, noise_sigma, proc_noise_sigma))[:-1, :]
times_data = _data[:, 0]
...
anneal1.save_paths("../../../data/oc-dspe/ML/adj/proc_noise/%s/"
					"/all_sigma=%.1f_proc_noise_sigma=%.1f_paths_%d.npy" 
					% (I_type, noise_sigma, proc_noise_sigma, seed))
anneal1.save_params("../../../data/oc-dspe/ML/adj/proc_noise/%s/"
					"all_sigma=%.1f_proc_noise_sigma=%.1f_params_%d.npy" 
					% (I_type, noise_sigma, proc_noise_sigma, seed))
anneal1.save_action_errors("../../../data/oc-dspe/ML/"
					"adj/proc_noise/%s/all_sigma=%.1f_proc_noise_sigma=%.1f_action_errors_%d.npy" \
					% (I_type, noise_sigma, proc_noise_sigma, seed))
