...
uses a tighter bounds on the parameters.
...
np.random.seed(seed)
P0 = [np.array([np.random.uniform(0, 50)])]*3 + \
...
anneal1.save_paths("../../../data/oc-dspe/ML/dspe/%s/tight"
					"/sigma=%.1f_paths_%d.npy" % (I_type, noise_sigma, seed))
anneal1.save_params("../../../data/oc-dspe/ML/dspe/%s/tight/"
					"sigma=%.1f_params_%d.npy" % (I_type, noise_sigma, seed))
anneal1.save_action_errors("../../../data/oc-dspe/ML/"
					"dspe/%s/tight/sigma=%.1f_action_errors_%d.npy" \
